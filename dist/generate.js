...
const generators = {
    string: (pathDef, { options }) => {
        ...
        if ((0, types_1.isStringFieldOptions)(options)) {
            switch (options.type) {
                case 'email':
                    return chance.email();
                case 'firstname':
    ...
    array: (pathDef, { options, staticFields, globalOptions }) => {
        ...
        if (typeof pathDef.caster === 'object') {
            ...
            return Array.from(Array(num)).map(() => generator(pathDef.caster, { options, globalOptions }));
        }
        if (typeof pathDef.caster === 'function') {
            // eslint-disable-next-line no-use-before-define
            return Array.from(Array(num)).map(() => generate(pathDef.schema, { options, staticFields, globalOptions }));
        }
        return null;
    },
    objectid: (pathDef, { options, globalOptions }) => {
        if ((0, types_1.isPopulateWithFactory)(options)) {
            return options.populateWithFactory.generate();
        }
        else if ((0, types_1.isPopulateWithSchema)(options)) {
            const mockGen = (0, mocker_1.factory)(options.populateWithSchema);
        ...
        if (get(options, 'tostring', OBJECT_ID_STRINGIFY_BY_DEFAULT) === false || get(globalOptions, 'objectid.tostring', OBJECT_ID_STRINGIFY_BY_DEFAULT) === false) {
            return new bson_objectid_1.default();
        }
        return new bson_objectid_1.default().toString();
    },
    number: (pathDef) => {
    ...
    map: (pathDef, { options, globalOptions }) => {
        ...
        const mock = keys.reduce((obj, key) => Object.assign(obj, { [key]: generator(pathDef, { options, globalOptions }) }), {});
        return mock;
    },
};
...
function populateField(mockObject, { schema, path, options, staticValue, indirectValues = {}, globalOptions = {}, }) {
    const fieldOptions = getPropertyByName(options, path) || getPropertyByPath(options, path) || {};
    if (!shouldSkipProperty(path, options)) {
        ...
        if (staticValue !== undefined) {
            ...
            value = generator(pathDef, { options: fieldOptions, staticFields: staticValue, globalOptions });
        }
    ...
    schema.eachPath((path) => {
        if (!path.includes('$*')) {
            if (typeof get(options, `${path}.value`) === 'function' || typeof get(options, `['${path}'].value`, undefined) === 'function') {
                delayedFieldGeneration.push(() => populateField(mockObject, {
                ...
                fieldGeneration.push(() => populateField(mockObject, {
                    schema,
                    path,
                    options,
